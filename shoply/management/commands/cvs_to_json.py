...
class Command(BaseCommand):
    help = (
        'Convert a CSV file to JSON with an updated timestamped '
        'filename and save it in the data/json folder'
    )

    def handle(self, *args, **kwargs):
        # Step 1: Prompt for the CSV file path
        csv_file_path = input(
            "Enter the path of the CSV file (e.g., "
        ...
        logger.info(
            ...
            f"New JSON file: {json_file_path}"
        )
        ...
        try:
            ...
            logger.info(
                    f"Successfully converted CSV to JSON: {json_file_path}"
                )
    ...
    def add_or_replace_timestamp(file_name):
        """
        Checks if the filename starts with a timestamp
        and replaces it with the current timestamp.
        ...
        for row in csv_reader:
            ...
            product_entry = {
                ...
                "fields": {
                    ...
                    "boughtInLastMonth": (
                            row['boughtInLastMonth'].lower() == 'true'
                        ),
